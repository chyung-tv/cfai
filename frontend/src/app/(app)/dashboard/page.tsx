  ...
  useEffect(() => {
    ...
    const updateQueries = async () => {
      for (const query of queries) {
        ...
        const lifecycleMessage = event.message || event.status;

        // Check if stream indicates completion
        if (
          lifecycleMessage === "Analysis completed" ||
        ...
        if (
          lifecycleMessage.toLowerCase().includes("error") ||
          lifecycleMessage.toLowerCase().includes("failed")
        ) {
